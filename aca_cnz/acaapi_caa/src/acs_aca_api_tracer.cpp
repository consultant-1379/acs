...
int __CLASS_NAME__::vtrace (const char * file, int line, const char * function, const char * format, va_list ap) {
	if (_tra_tracer.ACS_TRA_ON()) {
		char trace_msg[ACS_ACA_TRACE_MESSAGE_SIZE_MAX];
		int char_wrote = 0;

		if (file || function || (line > 0)) {
			char_wrote = ::snprintf(trace_msg, ACS_ACA_ARRAY_SIZE(trace_msg), "(%06d) {", (pid_t) syscall (SYS_gettid));

			if (function) {
				char_wrote += ::snprintf(trace_msg + char_wrote, ACS_ACA_ARRAY_SIZE(trace_msg) - char_wrote, "%s, ", function);
			}
			if (file) {
				const char * filename = ::strrchr(file, '/');
				char_wrote += ::snprintf(trace_msg + char_wrote, ACS_ACA_ARRAY_SIZE(trace_msg) - char_wrote, "%s", filename ? (filename + 1) : "NO_FILE");
			}

			(line > 0) && (char_wrote += ::snprintf(trace_msg + char_wrote, ACS_ACA_ARRAY_SIZE(trace_msg) - char_wrote, "::%d", line));
			char_wrote += ::snprintf(trace_msg + char_wrote, ACS_ACA_ARRAY_SIZE(trace_msg) - char_wrote, "} ");
		}

		::vsnprintf(trace_msg + char_wrote, ACS_ACA_ARRAY_SIZE(trace_msg) - char_wrote, format, ap);
		trace_msg[ACS_ACA_ARRAY_SIZE(trace_msg) - 1] = 0;
...
__CLASS_NAME__::__CLASS_NAME__ (acs_aca_tracer & tracer, const char * function_name)
...
__CLASS_NAME__::__CLASS_NAME__ (acs_aca_tracer & tracer, const char * function_name, const char * file_name, int line)
