...
ACS_SCH_Event* ACS_SCH_Event::instance()
{
    return instance_::instance();
}


ACS_SCH_EventHandle ACS_SCH_Event::CreateEvent(bool manualReset, bool initialState, const char* name)
{
    ACS_SCH_EventHandle eventHandle = -1;
...
ACS_SCH_EventHandle ACS_SCH_Event::OpenNamedEvent(string name)
{
    ...
    ACS_SCH_EventHandle handle = it->second->handle;
    instance()->lock_.release();
...
void ACS_SCH_Event::CloseEvent(ACS_SCH_EventHandle eventHandle)
{
    instance()->lock_.acquire_write();
...
int ACS_SCH_Event::WaitForEvents(int count, int *eventHandles, int timeoutInMilliseconds)
{
    fd_set fds;
    int nfds = 0;
    ...
    if (ret == 0) {
        ...
        return ACS_SCH_Event_RC_TIMEOUT;
    ...
    if (n != -1)
    {
        ...
        ACS_SCH_Event::EventProperties_t *ep = NULL;

        // locate what type of event this is, start in the named events list
        instance()->lock_.acquire_read();
        ACS_SCH_Event::NamedEventIteratorType neit = instance()->findNamedEventByHandle(handleActivated);
        instance()->lock_.release();
        ...
        if (ep == NULL) {
            ...
            instance()->lock_.acquire_read();
            ACS_SCH_Event::EventIteratorType ueit = instance()->findEventByHandle(handleActivated);
            instance()->lock_.release();
...
bool ACS_SCH_Event::SetEvent(ACS_SCH_EventHandle eventHandle)
{
    ssize_t sz;
    uint64_t n = 1;
    ...
    if (sz != sizeof(uint64_t)) {
        // write fault
        //ACS_CS_TRACE((ACS_SCH_Event_TRACE, "(%t) Failed to set event for handle: %d", eventHandle));
        return false;
    }
...
bool ACS_SCH_Event::ResetEvent(ACS_SCH_EventHandle eventHandle)
{
    fd_set fds;
    struct timeval tv;
...
ACS_SCH_Event::NamedEventIteratorType ACS_SCH_Event::findNamedEventByHandle(ACS_SCH_EventHandle handle)
{
    // check the list with named events
...
// NOTE! Must be wrapped inside acquire/release statement
ACS_SCH_Event::EventIteratorType ACS_SCH_Event::findEventByHandle(ACS_SCH_EventHandle handle)
{
    // check the list of unnamed events
