...
ACS_TRAPDS_HA_AppManager::ACS_TRAPDS_HA_AppManager(const char* daemon_name):ACS_APGCC_ApplicationManager(daemon_name){
        /* create the pipe for shutdown handler */
        Is_terminated = FALSE;
        startSRV = NULL;
        //startSRV = new (std::nothrow) ACS_TRAPDS_Server();
        ...
        //ACS_TRAPDS_logging = ACS_TRAPDS_Logger::getLogInstance();


}

ACS_TRAPDS_HA_AppManager::~ACS_TRAPDS_HA_AppManager(){
	//destroy dynamic allocations

}


ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performStateTransitionToActiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState){
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performStateTransitionToPassiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState) {
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performStateTransitionToQueisingJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performStateTransitionToQuiescedJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
        (void)previousHAState;
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performComponentHealthCheck(void)
{
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performComponentTerminateJobs(void)
{
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performComponentRemoveJobs(void)
{
...
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::performApplicationShutdownJobs() {
...
ACE_THR_FUNC_RETURN svc_run(void *ptr){
	ACS_TRAPDS_HA_AppManager *haObj = (ACS_TRAPDS_HA_AppManager*) ptr;
        haObj->svc();
        return 0;
}
ACS_APGCC_ReturnType ACS_TRAPDS_HA_AppManager::svc(){
        ...
        ACE_INT32 retCode;
        //ACS_TRAPDS_Server *startSRV = new (std::nothrow) ACS_TRAPDS_Server();
        ...
        while(true)
        {
                ...
                if (fds[0].revents & POLLIN){
                        ...
                        if (ha_state[0] == 'S'){
                        	...
                        	ACS_TRAPDS_EventHandle handle = ACS_TRAPDS_Event::OpenNamedEvent(TRAPDS_Util::EVENT_NAME_SHUTDOWN);
                        	(void) ACS_TRAPDS_Event::SetEvent(handle);

                        	if(startSRV){
                        		sleep(TRAPDS_Util::Daemon_Timeout);
                        		delete startSRV;
                        		startSRV = NULL;
                        	}
                        	ACS_TRAPDS_Event::ResetEvent(handle);
