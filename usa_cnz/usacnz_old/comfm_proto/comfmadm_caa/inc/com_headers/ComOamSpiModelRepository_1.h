...
typedef enum ComOamSpiStatus {
    ...
    /**
     * The definition is obsolete, but it permits new or continued
...
/**    static ComOamSpiGeneralProperties_2T * initInterface();
    GeneralPropertiesIf2Impl(GeneralPropertiesWrapper * generalPropertiesWrapper);
    ~GeneralPropertiesIf2Impl();

    // ----------- interface methods
    ComReturnT getName(const char** name);

    ComReturnT getDescription(const char** description);

    ComReturnT getSpecification(const char** specification);

    ComReturnT getStatus(ComOamSpiStatus_2T* status);

    ComReturnT getHidden(const char** hidden);

    ComReturnT getDomain(const char** domain);

    ComReturnT getExtension(ComOamSpiExtensionHandleT* subHandle);
