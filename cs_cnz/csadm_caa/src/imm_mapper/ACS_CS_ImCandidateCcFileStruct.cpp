/*
 * ACS_CS_ImCandidateCcFileStruct.cpp
 *
...
BOOST_CLASS_EXPORT_IMPLEMENT(ACS_CS_ImCandidateCcFileStruct);

ACS_CS_ImCandidateCcFileStruct::ACS_CS_ImCandidateCcFileStruct():
axeCpClusterStructId(""),actionId(UNDEFINED_TYPE),state(UNDEFINED_STATE),
result(NOT_AVAILABLE),reason(""), timeOfLastAction("")
{
   type = CANDIDATECCFILESTRUCT_T;
}

ACS_CS_ImCandidateCcFileStruct::ACS_CS_ImCandidateCcFileStruct(const ACS_CS_ImCandidateCcFileStruct &other)
{
   *this = other;
}
...
bool ACS_CS_ImCandidateCcFileStruct::isValid(int & errNo, const ACS_CS_ImModel &/*model*/)
{
   errNo = 0;
   return true;
}

bool ACS_CS_ImCandidateCcFileStruct::modify(ACS_APGCC_AttrModification **attrMods)
{
...
ACS_CS_ImBase * ACS_CS_ImCandidateCcFileStruct::clone() const
{
   ACS_CS_ImBase *bladeClusterInfo = new ACS_CS_ImCandidateCcFileStruct(*this);
   return bladeClusterInfo;
}


ACS_CS_ImCandidateCcFileStruct & ACS_CS_ImCandidateCcFileStruct::operator=(const ACS_CS_ImCandidateCcFileStruct &rhs)
{
   copyObj(rhs);
   return *this;
}
...
ACS_CS_ImCandidateCcFileStruct & ACS_CS_ImCandidateCcFileStruct::copyObj(const ACS_CS_ImCandidateCcFileStruct &object)
{
...
bool ACS_CS_ImCandidateCcFileStruct::equals(const ACS_CS_ImBase &obj) const
{
   if (!ACS_CS_ImBase::equals(obj))
      return false;

   try {
      const ACS_CS_ImCandidateCcFileStruct & other = dynamic_cast<const ACS_CS_ImCandidateCcFileStruct &>(obj);
...
ACS_CS_ImImmObject * ACS_CS_ImCandidateCcFileStruct::toImmObject(bool onlyModifiedAttrs)
{

   ACS_CS_ImImmObject *immObject = new ACS_CS_ImImmObject();

   immObject->setClassName(ACS_CS_ImmMapper::CLASS_CANDIDATE_STRUCT.c_str());
...
namespace boost {
   namespace serialization {
      template void serialize( boost::archive::text_iarchive & ar, ACS_CS_ImCandidateCcFileStruct & base, const unsigned int /*version*/);
      template void serialize( boost::archive::text_oarchive & ar, ACS_CS_ImCandidateCcFileStruct & base, const unsigned int /*version*/);
      template void serialize( boost::archive::binary_iarchive & ar, ACS_CS_ImCandidateCcFileStruct & base, const unsigned int /*version*/);
      template void serialize( boost::archive::binary_oarchive & ar, ACS_CS_ImCandidateCcFileStruct & base, const unsigned int /*version*/);
   }
}
