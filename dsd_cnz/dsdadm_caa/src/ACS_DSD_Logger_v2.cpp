...
ACS_TRA_LogResult __CLASS_NAME__::vlogf (int sys_errno, int syslog_priority, ACS_TRA_LogLevel level,
	...
	char_count = ::vsnprintf(buffer, ACS_DSD_ARRAY_SIZE(buffer), format, ap);

	if (sys_errno ^ ERRNO_DISABLED_MASK) { // Add system errno information to the log message
		char errno_buf[1024];
		::snprintf(buffer + char_count, ACS_DSD_ARRAY_SIZE(buffer) - char_count, " [errno == %d, errno_text == '%s']",
				sys_errno, ::strerror_r(sys_errno, errno_buf, ACS_DSD_ARRAY_SIZE(errno_buf)));
	}
...
ACS_TRA_LogResult __CLASS_NAME__::vlogf_source (int sys_errno, int syslog_priority,
	...
	char_count = ::snprintf(buffer, ACS_DSD_ARRAY_SIZE(buffer), "{%s@@%d} ", source_func_name, source_line);
