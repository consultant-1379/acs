...
#
#
# Author : XPHAVAL, TEIGGAM, ESALVES, EANBUON
# Version 2.2
#
...
create_campaign()
{
        ...
        do
			...
			# ---- Including for more than one service
                        ...
                        sed "s/$tagSG/$sgT$safSg$safApp/g" "$f" > $TFILE && mv -f $TFILE "$f"
			...
			sed "s/$tagSIRank/$SiRank/g" "$f" > $TFILE && mv -f $TFILE "$f"
                        # -----------------------------
