...
void CreateRuntimeObjectNotCached(ObjectHandler *obj) {
	...
	/*Create the class*/
//
	//11ASSERTM("CUTE: ACS_APGCC_RuntimeOwner:createRuntimeObj method ", returnCode == ACS_CC_SUCCESS);
...
void actionTest(ObjectHandler *obj){
...
//	vectorIN.push_back(secondElem);
	...
	if(returnCode != ACS_CC_SUCCESS){
		cout<<"Error Initialize OmHandler"<<endl;
		cout<<"ERROR CODE: "<<admOp.getInternalLastError()<<endl;
	}

	int adminOpreturnValue;
	returnCode = admOp.adminOperationInvoke(objName_2, 0, 5, vectorIN, &adminOpreturnValue, timeOutVal_5sec);
	...
	returnCode = admOp.adminOperationInvoke(objName_2, 0, 100, vectorIN, &adminOpreturnValue, timeOutVal_10sec);
...
void actionTestWithRetPar(ObjectHandler *obj){
	...
	/*Param for AdminOperation*/
	long long int timeOutVal_5sec = 5*(1000000000LL);
	long long int timeOutVal_15sec = 15*(1000000000LL);

	cout << " -----------------------------------------------------------------------------------" << endl;
	cout << " actionTest With Return Parameters: 2 tests " << endl;
	cout << " TEST1: invoke admin operation expecting return parameters too   " << endl;
	cout << " RESULT1: ok parameters are retrieved after ivoke succedes \n" << endl;
	cout << " TEST2: invoke admin operation expecting return parameters too expected to fail for timeout" << endl;
	cout << " RESULT2: invke fails for timeout \n" << endl;
	cout << " ----------------------------------------------------------------------------------- " << endl;
	...
	/* Send invoke: expected to be successfull*/
	returnCode = admOp.adminOperationInvoke(objName_2, 0, 11, vectorIN, &adminOpreturnValue, timeOutVal_15sec, outparamVector);

	std::cout <<" Hereafter the returnCode: "<< returnCode << std::endl;
	std::cout <<" Hereafter the admOp.getInternalLastError(): "<< admOp.getInternalLastError() << std::endl;

	ASSERTM("1.5.1 actionTestWithRetPar:admOp.adminOperationInvoke check result ", returnCode ==  ACS_CC_SUCCESS);
	ASSERTM("1.5.1 Test_1:admOp.adminOperationInvoke check result ", adminOpreturnValue == 1 );

	std::cout <<" outputparamvectorsize: "<< outparamVector.size() << std::endl;

	if ( outparamVector.size() !=0 ){
		std::cout <<" Hereafter the received return parameters ..." << std::endl;
		for(unsigned int k= 0; k<outparamVector.size();k++){
			std::cout <<" Hereafter the received return parameters ..." << std::endl;
			std::cout <<"Param "<< k+1 << ")" << std::endl;

			switch ( outparamVector[k].attrType ) {
				case ATTR_INT32T:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " <<*reinterpret_cast<int *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_UINT32T:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " <<*reinterpret_cast<unsigned int *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_INT64T:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " <<*reinterpret_cast<long long *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_UINT64T:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " <<*reinterpret_cast<unsigned long long *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_FLOATT:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " <<*reinterpret_cast<float *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_DOUBLET:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " <<*reinterpret_cast<double *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_NAMET:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " << reinterpret_cast<char *>(outparamVector[k].attrValues) << endl;
					break;
				case ATTR_STRINGT:
					cout << " paramName: " << outparamVector[k].attrName << "\t value: " << reinterpret_cast<char *>(outparamVector[k].attrValues) << endl;
					break;
		...
		std::cout <<" End the received return parameters list " << std::endl;
		admOp.freeadminOperationReturnValue ( outparamVector );
	}

	sleep(1);

	/* Send invoke: expected to faili because of timeout*/
		returnCode = admOp.adminOperationInvoke(objName_2, 0, 12, vectorIN, &adminOpreturnValue, timeOutVal_5sec, outparamVector);

		//std::cout <<" Hereafter the returnCode: "<< returnCode << std::endl;
		//std::cout <<" Hereafter the admOp.getInternalLastError(): "<< admOp.getInternalLastError() << std::endl;

		ASSERTM("1.5.1 actionTestWithRetPar:admOp.adminOperationInvoke check result ", returnCode ==  ACS_CC_FAILURE);
		ASSERTM("1.5.1 Test_1:admOp.adminOperationInvoke check result ", admOp.getInternalLastError() == -5 );
	...
	std::cout <<" In TestADM OP  stopping thread" << std::endl;
	th.stop();
	sleep(10);
