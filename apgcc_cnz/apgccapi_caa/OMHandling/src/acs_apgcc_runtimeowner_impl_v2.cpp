/*
 * acs_apgcc_runtimeowner_impl_v2.cpp
 *
...
namespace {
	ACS_TRA_trace trace(const_cast<char*>("ACS_APGCC_RuntimeOwner_V2"), const_cast<char *>("C300"));
}

map<SaUint64T,ACS_APGCC_RuntimeOwner_V2 *> acs_apgcc_runtimeowner_impl_v2::implementerList;



acs_apgcc_runtimeowner_impl_v2::acs_apgcc_runtimeowner_impl_v2() {
...
acs_apgcc_runtimeowner_impl_v2::acs_apgcc_runtimeowner_impl_v2(acs_apgcc_runtimeowner_impl_v2 *runtimeImp){
...
ACS_CC_ReturnType acs_apgcc_runtimeowner_impl_v2::init(std::string impName, ACS_APGCC_RuntimeOwner_V2 *p_runtimeOwn){

	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::init(std::string impName)");
	...
	//version.minorVersion =  11;/*for Ticket 1764*/
	...
	immCallbacks.saImmOiRtAttrUpdateCallback = acs_apgcc_runtimeowner_impl_v2::OiRtAttUpdateCallback;
	immCallbacks.saImmOiAdminOperationCallback = acs_apgcc_runtimeowner_impl_v2::adminOperationCallback;
	...
	if ( errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::init():saImmOiInitialize_2:FAILED", LOG_LEVEL_ERROR);
	...
	if( errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::init():saImmOiImplementerSet:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
		...
		errorCode = saImmOiFinalize(immOiHandle);
			...
			if (errorCode != SA_AIS_OK){
				...
				log.Write("ACS_APGCC_RuntimeOwner_V2::init():saImmOiFinalize:FAILED", LOG_LEVEL_ERROR);
				log.Write(buffer, LOG_LEVEL_ERROR);
	...
	if( errorCode != SA_AIS_OK ){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::init():saImmOiSelectionObjectGet:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
ACS_CC_ReturnType acs_apgcc_runtimeowner_impl_v2::finalize(){
	...
	if (errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::finalize():saImmOiImplementerClear:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
	...
	if (errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::finalize():saImmOiFinalize:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
ACS_CC_ReturnType acs_apgcc_runtimeowner_impl_v2::createRuntimeObj(const char* p_className, const char* p_parentName,
		vector<ACS_CC_ValuesDefinitionType> m_attrValuesList){


	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::createRuntimeObj(const char* p_className, const char* p_parentName, vector<ACS_CC_ValuesDefinitionType> m_attrValuesList)");
		trace.ACS_TRA_event(1, buffer);
	}
	...
	if (errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::createRuntimeObj():saImmOiRtObjectCreate_2:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
ACS_CC_ReturnType acs_apgcc_runtimeowner_impl_v2::deleteRuntimeObj(const char *p_objName){


	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::deleteRuntimeObj(const char *p_objName)");
		trace.ACS_TRA_event(1, buffer);
	}
	...
	if((errorCode = saImmOiRtObjectDelete(immOiHandle, &objToDelete ))!= SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::deleteRuntimeObj():saImmOiRtObjectDelete:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
ACS_CC_ReturnType acs_apgcc_runtimeowner_impl_v2::modifyRuntimeObj(const char* p_objName, ACS_CC_ImmParameter* p_attributeValue){

	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::modifyRuntimeObj(const char* p_objName, ACS_CC_ImmParameter* p_attributeValue)");
		trace.ACS_TRA_event(1, buffer);
	}
	...
	if(errorCode!=SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::modifyRuntimeObj():saImmOiRtObjectUpdate_2:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
ACS_CC_ReturnType acs_apgcc_runtimeowner_impl_v2::dispatch(ACS_APGCC_DispatchFlags p_flag){

	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::dispatch(ACS_APGCC_DispatchFlags p_flag)");
		trace.ACS_TRA_event(1, buffer);
	}
	...
	if (errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::dispatch():saImmOiDispatch:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
ACS_CC_ReturnType  acs_apgcc_runtimeowner_impl_v2::adminOperationResult(ACS_APGCC_OiHandle oiHandle,ACS_APGCC_InvocationType invocation, int result){

	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::adminOperationResult(ACS_APGCC_OiHandle oiHandle,ACS_APGCC_InvocationType invocation, int result)");
		trace.ACS_TRA_event(1, buffer);
	}
	...
	if (errorCode != SA_AIS_OK){
		...
		log.Write("ACS_APGCC_RuntimeOwner_V2::adminOperationResult::saImmOiAdminOperationResult:FAILED", LOG_LEVEL_ERROR);
		log.Write(buffer, LOG_LEVEL_ERROR);
...
SaAisErrorT acs_apgcc_runtimeowner_impl_v2::OiRtAttUpdateCallback(SaImmOiHandleT immOiHandle,
			const SaNameT *objectName,
			const SaImmAttrNameT *attributeNames){

	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "ACS_APGCC_RuntimeOwner_V2::OiRtAttUpdateCallback(SaImmOiHandleT immOiHandle, const SaNameT *objectName, const SaImmAttrNameT *attributeNames)");
		trace.ACS_TRA_event(1, buffer);
	}
	...
	ACS_APGCC_RuntimeOwner_V2 *obj = (ACS_APGCC_RuntimeOwner_V2 *)(acs_apgcc_runtimeowner_impl_v2::implementerList.find(immOiHandle)->second);
	...
	returnCode = obj->updateCallback(nameObj, (const char**)(nameAttrs));
...
void acs_apgcc_runtimeowner_impl_v2::adminOperationCallback( SaImmOiHandleT immOiHandle, SaInvocationT invocation,
		const SaNameT *objectName, SaImmAdminOperationIdT operationId, const SaImmAdminOperationParamsT_2 **params){

	if (trace.ACS_TRA_ON()){
		char buffer[1024] = {0};
		sprintf(buffer, "acs_apgcc_runtimeowner_impl_v2::adminOperationCallback(SaImmOiHandleT immOiHandle,SaInvocationT invocation,const SaNameT *objectName,SaImmAdminOperationIdT operationId,const SaImmAdminOperationParamsT_2 **params)");
		trace.ACS_TRA_event(1, buffer);
	}
...
int acs_apgcc_runtimeowner_impl_v2::getSelObj() const{

	return (int)selObj;
}


int acs_apgcc_runtimeowner_impl_v2::getInternalLastError(){

	return ((-1)*errorCode);
}


void acs_apgcc_runtimeowner_impl_v2::setInternalError(int p_errorCode){
...
char* acs_apgcc_runtimeowner_impl_v2::getInternalLastErrorText(){
...
/*for Ticket 1764*/
ACS_CC_ReturnType  acs_apgcc_runtimeowner_impl_v2::adminOperationResult(ACS_APGCC_OiHandle oiHandle,ACS_APGCC_InvocationType invocation, int result, vector<ACS_APGCC_AdminOperationParamType> outParamVector){

	if (trace.ACS_TRA_ON()){
			char buffer[1024] = {0};
			sprintf(buffer, "acs_apgcc_runtimeowner_impl_v2::adminOperationResult(ACS_APGCC_OiHandle oiHandle,ACS_APGCC_InvocationType invocation, int result, vector<ACS_APGCC_AdminOperationParamType> outParamVector)");
			trace.ACS_TRA_event(1, buffer);
		}
		...
		if (errorCode != SA_AIS_OK){
			...
			log.Write("acs_apgcc_runtimeowner_impl_v2::adminOperationResult::saImmOiAdminOperationResult:FAILED", LOG_LEVEL_ERROR);
			log.Write(buffer, LOG_LEVEL_ERROR);
