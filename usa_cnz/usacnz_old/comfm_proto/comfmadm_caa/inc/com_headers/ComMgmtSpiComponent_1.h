...
typedef enum ComStateChangeReason {
    ...
    ComDeactivating = 2,
    /**
     * <p>COM changes either to active or unactive state due to a failover.
    ...
    ComFailingOver = 3,
    /**
     * <p>COM changes either to active or unactive state due to a switchover.
     * This reason is given only if COM is running in a high availability configuration.
...
typedef struct ComMgmtSpiComponent_1 {
     ...
     * A special case is if the component does not specify the component name when fetching
     * the interface from the COM Interface Portal (that is, setting the implementing ComponentName
     * to null). In this case, the dependency to this interface is specified in the same way
     * by setting the ComponentName to null.
     */
     ...
     * they are not retrieved from the COM Interface Portal before entering this function.
     *
