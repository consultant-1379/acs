...
def createSMFCampaignPlugin():
   return NSFSMFCampaignPlugin()

class NSFSMFCampaignPlugin(SMFCampaignPlugin):
   # initialization method
   # self: variable represents the instance of the object itself
   def __init__(self):
      super(NSFSMFCampaignPlugin, self).__init__()
      self.MY_COMPONENT_UID = "acs.nsf"
      self._info = None
      self._utils = None
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
   def cliAtCampInit(self):
      ...
      nsf = self._info.getComponent("acs.nsf")
      for bundle in nsf.swBundles:
         bundleName = bundle[1] 
      result = []
      ...
      if (self._actionType == SMFConstants.CT_INSTALL or self._actionType == SMFConstants.CT_UPGRADE or self._actionType == SMFConstants.CT_MIGRATE):
         cli = os.path.join("$OSAFCAMPAIGNROOT", self.MY_COMPONENT_UID, "scripts", "nsf_helper.sh" )
