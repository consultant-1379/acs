#include "ACS_DSD_ConfigParams.h"
#include "ACS_DSD_Imm_functions.h"
#include "ACS_DSD_HA_AppManager.h"
//#include "ACS_DSD_Logger.h"
#include "ACS_DSD_Logger_v2.h"

extern int gbl_dsdd_must_continue_work;						// defined in ACS_DSD_Main.cpp
extern int is_imm_shared_objects_controller;				// defined in ACS_DSD_Main.cpp. Indicates if this instance is the controller of shared IMM objects
extern ACS_DSD_Logger	dsd_logger;							// defined in ACS_DSD_Main.cpp. Used to log (by means of TRA API)
extern ACE_THR_FUNC_RETURN run_dsd_daemon_work(void *);		// defined in ACS_DSD_Main.cpp
extern void stop_dsd_daemon_work();							// defined in ACS_DSD_Main.cpp
extern bool dsd_is_stopping;								// defined in ACS_DSD_Main.cpp

ACE_THR_FUNC_RETURN svc_run(void *);

ACS_DSD_HA_AppManager::ACS_DSD_HA_AppManager(const char* daemon_name)
: ACS_APGCC_ApplicationManager(daemon_name)
{
	Is_terminated = FALSE;
	dsd_worker_thread_id = 0;
	dsd_ha_application_thread_id = 0;
	dsd_worker_thread_terminated = false;
	dsd_worker_thread_exit_code = -1;

	/* create the pipe for shutdown handler */
	if ( (pipe(readWritePipe)) < 0) {
		syslog(LOG_ERR, "pipe creation FAILED");
	}

	if ( (fcntl(readWritePipe[0], F_SETFL, O_NONBLOCK)) < 0) {
		syslog(LOG_ERR, "pipe fcntl on readn");
	}

	if ( (fcntl(readWritePipe[1], F_SETFL, O_NONBLOCK)) < 0) {
		syslog(LOG_ERR, "pipe fcntl on writen");
	}
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performStateTransitionToActiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
	syslog(LOG_INFO, "ACS_DSD_HA_AppManager::performStateTransitionToActiveJobs() invoked !" );

	/* Check if we have received the ACTIVE State again.
	 * This means that, our application is already Active and
	 * again we have got a callback from AMF to go active.
	 * Ignore this case anyway. This case should rarely happen
	 */
	ACE_TCHAR state[1] = {'A'};
	if(ACS_APGCC_AMF_HA_ACTIVE == previousHAState)
		return ACS_APGCC_SUCCESS;

	/* Our application has received state ACTIVE from AMF.
	 * Start off with the activities needs to be performed
	 * on ACTIVE
	 */

	/* Check if it is due to State Transition ( Undefined --> Active)*/
	if ( ACS_APGCC_AMF_HA_UNDEFINED != previousHAState )
	{
		syslog(LOG_INFO, "State Transition happened. Becoming Active now");
		/* Inform the HA Application thread to go "active" state */
		write(readWritePipe[1], &state, sizeof(state));
		return ACS_APGCC_SUCCESS;
	}

	/* Handle here what needs to be done (only the first time) when you are given ACTIVE State */
	syslog(LOG_INFO, "DSD Daemon received ACTIVE state assignment!!!");

	/* Create a thread (let's call it "HA Application Thread") with the state machine (active, passive, stop states)
	 * and start off with "active" state activities.
	 */

	// HA Application thread should not be already running ... !
	if(dsd_ha_application_thread_id != 0)
		syslog(LOG_WARNING, "Found an old HA Application Thread still running !!!");

	/* spawn thread */
	const ACE_TCHAR* thread_name = "HA_Application_Thread";
	int call_result = ACE_Thread_Manager::instance()->spawn(&svc_run,
									(void *)this ,
									THR_NEW_LWP | THR_JOINABLE | THR_INHERIT_SCHED,
									& dsd_ha_application_thread_id,
									0,
									ACE_DEFAULT_THREAD_PRIORITY,
									-1,
									0,
									ACE_DEFAULT_THREAD_STACKSIZE,
									&thread_name);
	if (call_result == -1)
	{
		syslog(LOG_ERR, "Error creating HA Application Thread");
		return ACS_APGCC_FAILURE;
	}

	/* Inform the HA Application thread to go "active" state */
	write(readWritePipe[1], &state, sizeof(state));

	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performStateTransitionToPassiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
	syslog(LOG_INFO, "ACS_DSD_HA_AppManager::performStateTransitionToPssiveJobs() invoked !" );

	/* Check if we have received the PASSIVE State Again.
	 * This means that, our application was already Passive and
	 * again we have got a callback from AMF to go passive.
	 * Ignore this case anyway. This case should rarely happen.
	 */

	ACE_TCHAR state[1] = {'P'};
	if(ACS_APGCC_AMF_HA_STANDBY == previousHAState)
		return ACS_APGCC_SUCCESS;

	/* Our application has received state PASSIVE from AMF.
	 * Check if the state received is due to State Transistion.
	 * (Active->Passive).
	 */
	if ( ACS_APGCC_AMF_HA_UNDEFINED != previousHAState ){
		syslog(LOG_INFO, "State Transition happened. Becoming Passive now");
		 /* Inform the thread to go "passive" state*/
		write(readWritePipe[1], &state, sizeof(state));
		return ACS_APGCC_SUCCESS;
	}

	/* Create a thread (let's call it "HA Application Thread) with the state machine (active, passive, stop states)
	 * and start off the thread with "passive" state.
	 */

	syslog(LOG_INFO, "DSD Daemon received STANDBY state assignment!!!");

	const ACE_TCHAR* thread_name = "HA_Application_Thread";
	int call_result = ACE_Thread_Manager::instance()->spawn(&svc_run,
									(void *)this ,
									THR_NEW_LWP | THR_JOINABLE | THR_INHERIT_SCHED,
									& dsd_ha_application_thread_id,
									0,
									ACE_DEFAULT_THREAD_PRIORITY,
									-1,
									0,
									ACE_DEFAULT_THREAD_STACKSIZE,
									&thread_name);
	if (call_result == -1)
	{
		syslog(LOG_ERR, "Error creating HA Application Thread");
		return ACS_APGCC_FAILURE;
	}

	 /* Inform the thread to go "passive" state*/
	write(readWritePipe[1], &state, sizeof(state));
	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performStateTransitionToQueisingJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
	syslog(LOG_INFO, "ACS_DSD_HA_AppManager::performStateTransitionToQueisingJobs() invoked !" );

	(void)previousHAState;
	ACE_TCHAR state[1] = {'S'};

	/* We were active and now losing active state due to some shutdown admin
	 * operation performed on our SU.
	 * Inform the thread to go to "stop" state
	 */

	syslog(LOG_INFO, "DSD Daemon received QUIESING state assignment!!!");

	/* Inform the thread to go "stop" state */
	if ( !Is_terminated )
		write(readWritePipe[1], &state, sizeof(state));

	Is_terminated = TRUE;

	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performStateTransitionToQuiescedJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
	syslog(LOG_INFO, "ACS_DSD_HA_AppManager::performStateTransitionToQuiescedJobs() invoked !" );

	(void)previousHAState;
	ACE_TCHAR state[1] = {'S'};

	/* We were Active and now losting Active state due to Lock admin
	 * operation performed on our SU.
	 * Inform the thread to go to "stop" state
	 */

	syslog(LOG_INFO, "DSD Daemon received QUIESCED state assignment!");

	/* Inform the thread to go "stop" state */
	if ( !Is_terminated )
		write(readWritePipe[1], &state, sizeof(state));

	Is_terminated = TRUE;

	// wait for HA application thread termination
	int retval = wait_ha_application_thread_termination(false);
	if(retval < 0)
	{
		syslog(LOG_ERR, "an error occurred while waiting for HA Application Thread termination. ERRCODE == '%d'!", retval);
		return ACS_APGCC_FAILURE;
	}

	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performComponentHealthCheck(void)
{

	/* Application has received health check callback from AMF. Check the
	 * sanity of the application and reply to AMF that you are ok.
	 */
	//syslog(LOG_INFO, "DSD Daemon received healthcheck query!!!");

	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performComponentTerminateJobs(void)
{
	/* Application has received terminate component callback due to
	 * LOCK-INST admin operation performed on SU. Terminate the thread if
	 * we have not terminated in performComponentRemoveJobs case or double
	 * check if we are done so.
	 */
	syslog(LOG_INFO, "DSD Daemon received terminate callback!!!");

	ACE_TCHAR state[1] = {'S'};

	/* Inform the thread to go "stop" state */
	if ( !Is_terminated )					// TR HX65517 (DSD component restart timeout with amf-adm restart)
		write(readWritePipe[1], &state, sizeof(state));

	// wait for HA application thread termination
	int retval = wait_ha_application_thread_termination(false);// TR HX65517 
	if(retval < 0)
	{
		syslog(LOG_ERR, "an error occurred while waiting for HA Application Thread termination. ERRCODE == '%d'!", retval);
		return ACS_APGCC_FAILURE;
	}

	Is_terminated = FALSE;  // TR HX28206 
	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performComponentRemoveJobs(void)
{

	/* Application has received Removal callback. State of the application
	 * is neither Active nor Standby. This is with the result of LOCK admin operation
	 * performed on our SU. Terminate the thread by informing the thread to go "stop" state.
	 */

	ACE_TCHAR state[1] = {'S'};

	syslog(LOG_INFO, "Application Assignment is removed now");
	/* Inform the thread to go "stop" state */
	if ( !Is_terminated )
		write(readWritePipe[1], &state, sizeof(state));

	// wait for HA application thread termination
	int retval = wait_ha_application_thread_termination(false);
	if(retval < 0)
	{
		syslog(LOG_ERR, "an error occurred while waiting for HA Application Thread termination. ERRCODE == '%d'!", retval);
		return ACS_APGCC_FAILURE;
	}

	Is_terminated = FALSE; // reset attribute

	return ACS_APGCC_SUCCESS;
}


ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::performApplicationShutdownJobs() {

	syslog(LOG_INFO, "Shutting down the application");
	ACE_TCHAR state[1] = {'S'};

	if ( !Is_terminated )
		write(readWritePipe[1], &state, sizeof(state));

	// wait for HA application thread termination
	int retval = wait_ha_application_thread_termination(false);
	if(retval < 0)
	{
		syslog(LOG_ERR, "an error occurred while waiting for HA Application Thread termination. ERRCODE == '%d'!", retval);
		return ACS_APGCC_FAILURE;
	}

	Is_terminated = FALSE;  // reset attribute

	return ACS_APGCC_SUCCESS;
}


ACE_THR_FUNC_RETURN svc_run(void *ptr){
	ACS_DSD_HA_AppManager *haObj = (ACS_DSD_HA_AppManager*) ptr;
	haObj->svc();
	return 0;
}

ACS_APGCC_ReturnType ACS_DSD_HA_AppManager::svc()
{
	struct pollfd fds[1];
	nfds_t nfds = 1;
	ACE_INT32 ret;
	ACE_Time_Value timeout;
	ACE_INT32 retCode;

	syslog(LOG_INFO, "Starting HA Application Thread");

	__time_t secs = 5;
	__suseconds_t usecs = 0;
	timeout.set(secs, usecs);

	fds[0].fd = readWritePipe[0];
	fds[0].events = POLLIN;

	while(true)
	{
		ret = ACE_OS::poll(fds, nfds, &timeout); // poll can also be a blocking call, such case timeout = 0

		if (ret == -1)
		{
			if (errno == EINTR)
				continue;
			syslog(LOG_ERR,"poll Failed - %s, Exiting...", strerror(errno));
			kill(getpid(), SIGTERM);
			return ACS_APGCC_FAILURE;
		}

		if (ret == 0)
		{
		//	syslog(LOG_INFO, "timeout on ACE_OS::poll");
			continue;
		}

		if (fds[0].revents & POLLIN){
			ACE_TCHAR ha_state[1] = {'\0'};
			ACE_TCHAR* ptr = (ACE_TCHAR*) &ha_state;
			ACE_INT32 len = sizeof(ha_state);

			while (len > 0)
			{
				retCode=read(readWritePipe[0], ptr, len);
				if ( retCode < 0 && errno != EINTR)
				{
					syslog(LOG_ERR, "Read interrupted by error: [%s]",strerror(errno));
					kill(getpid(), SIGTERM);
					return ACS_APGCC_FAILURE;
				}
				else
				{
					ptr += retCode;
					len -= retCode;
				}

				if (retCode == 0)
					break;
        	}

			if ( len != 0)
			{
                syslog(LOG_ERR, "Improper Msg Len Read [%d]", len);
                kill(getpid(), SIGTERM);
                return ACS_APGCC_FAILURE;
        	}
			len = sizeof(ha_state);

			if (ha_state[0] == 'A')
			{
				syslog(LOG_INFO, "HA Application Thread: Application is Active");
				ACS_DSD_LOG(dsd_logger, LOG_LEVEL_INFO, ACS_DSD_TEXT("HA Application Thread: Received a notification for Active state!"));
				Is_terminated = FALSE;
				dsd_worker_thread_terminated = false;
				dsd_worker_thread_exit_code = -1;
				dsd_worker_thread_id = 0;
				dsd_is_stopping = false;
			}
			else if (ha_state[0] == 'P')
			{
				syslog(LOG_INFO, "HA Application Thread: Application is Passive");
			}
			else if (ha_state[0] == 'S')
			{
				syslog(LOG_INFO, "HA Application Thread: Request to stop application");

				/* Request to stop the thread, perform the graceful termination activities here */
				ACS_DSD_LOG(dsd_logger, LOG_LEVEL_INFO, ACS_DSD_TEXT("HA Application Thread: Received a request for stopping DSD Daemon!"));

				// stop DSD Daemon work
				stop_dsd_daemon_work();

				// wait Worker Thread termination
				syslog(LOG_INFO, "HA Application Thread: waiting for root DSD Worker Thread termination ... !");
				ACE_Thread_Manager::instance()->join(dsd_worker_thread_id);
				syslog(LOG_INFO, "HA Application Thread: root DSD Worker Thread terminated !");

				// exit from while loop and terminate
				break;
			}

			if(dsd_worker_thread_id == 0 )
			{
				/* create the root DSD worker thread */
				const ACE_TCHAR* thread_name = "Root_DSD_Worker_Thread";
				int call_res = ACE_Thread_Manager::instance()->spawn(& run_dsd_daemon_work,
														(void *)this,
														THR_NEW_LWP | THR_JOINABLE | THR_INHERIT_SCHED,
														& dsd_worker_thread_id,
														0,
														ACE_DEFAULT_THREAD_PRIORITY,
														-1,
														0,
														ACE_DEFAULT_THREAD_STACKSIZE,
														&thread_name);
				if (call_res == -1)
				{
					syslog(LOG_ERR, "Error creating root DSD Worker Thread");
					return ACS_APGCC_FAILURE;
				}


				syslog(LOG_INFO, "HA Application Thread: the root DSD Worker Thread has been successfully created !");
			}
		}
	}

	syslog(LOG_INFO, "HA Application Thread terminated successfully");
	return ACS_APGCC_SUCCESS;
}


void ACS_DSD_HA_AppManager::notify_worker_thread_termination(int exit_code)
{
	if(exit_code != DSD_SERVER_EXIT_CODE_OK)
	{
		syslog(LOG_INFO, "root DSD Worker Thread terminated with exit code < %d > !!!", exit_code);

		// request component restart to AMF
		if((exit_code != DSD_SERVER_EXIT_CODE_INTERRUPTED) && (componentReportError(ACS_APGCC_COMPONENT_RESTART) != ACS_APGCC_SUCCESS))
			exit(3);
	}

	dsd_worker_thread_exit_code = exit_code;
	dsd_worker_thread_terminated = true;
}

int ACS_DSD_HA_AppManager::wait_ha_application_thread_termination(bool nolog)
{
	if(!nolog) syslog(LOG_INFO, "Waiting for HA Application Thread termination ... ");

	if(dsd_ha_application_thread_id == 0)
	{
		if(!nolog) syslog(LOG_INFO, "HA Application Thread already terminated !");
		return 1; // HA application thread already terminated
	}

	int retval = ACE_Thread_Manager::instance()->join(dsd_ha_application_thread_id);
	if(retval == 0)
	{
		if(!nolog) syslog(LOG_INFO, "HA Application Thread successfully joined ! ");
		dsd_ha_application_thread_id = 0;  // HA application thread has terminated
	}
	else if(!nolog)
		syslog(LOG_ERR, "Failure joining HA Application Thread !");

	return retval;
}


