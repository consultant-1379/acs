...
###########################################################################
# :Title        Command cpgls, cpgdef, cpgrm
#         
# -------------------------------------------------------------------------
...
# ------------------------
# Test cpgls, cpgdef, cpgrm commands
#
...
if {[ath_send "hwcadd -m $Mag -s $SlotCP0 -n CPUB -t CP -y 0 -e A -d normal"]} {
    ath_tc_failed "hwcadd: addition of CP0 board failed"
}

#Verify that CP0 board has been added to the HWC table 
ath_send "hwcls"
set LastResponse $ATH::LastResponse
if {[ath_match $PatternHWC_CP $LastResponse]} {
    ath_tc_failed "hwcls: Printout does not match the expected one. \n"
}


#Verify that CP0 board has been added to CPG table  
ath_send "cpgls"
set LastResponse $ATH::LastResponse
if {[ath_match $PatternCPG_CP $LastResponse]} {
    ath_tc_failed "cpgls: Printout does not match the expected one. \n"
}
...
#Verify that BC0 and BC1 boards have been added to the HWC table 
ath_send "hwcls"
set LastResponse $ATH::LastResponse
...
#Verify that BC0 and BC1 board have been added to CPG table  
ath_send "cpgls"
set LastResponse $ATH::LastResponse
...
#Verify that BCG1 group has been added to CPG table  
ath_send "cpgls"
set LastResponse $ATH::LastResponse
...
#Remove the CP group containing BC0 and BC1 boards  
if {[ath_send "cpgrm $GroupName"]} {
    ath_tc_failed "cpgrm: removing CP group failed"
}


#Verify that BCG1 group has been removed from CPG table  
ath_send "cpgls"
set LastResponse $ATH::LastResponse
if {[ath_match $PatternCPG_all_nogroup $LastResponse]} {
    ath_tc_failed "cpgls: Printout does not match the expected one. \n"
