...
def createSMFCampaignPlugin():
   return EMFSMFCampaignPlugin()

class EMFSMFCampaignPlugin(SMFCampaignPlugin):
   # initialization method
   # self: variable represents the instance of the object itself
   def __init__(self):
      super(EMFSMFCampaignPlugin, self).__init__()
      self.MY_COMPONENT_UID = "acs.emf"
      self._info = None
      self._utils = None
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
   def cliAtCampInit(self):
      ...
      emf = self._info.getComponent("acs.emf")
      for bundle in emf.swBundles:
         bundleName = bundle[1] 
      result = []
      ...
      if (self._actionType == SMFConstants.CT_INSTALL or self._actionType == SMFConstants.CT_UPGRADE or self._actionType == SMFConstants.CT_MIGRATE):
         cli = os.path.join("$OSAFCAMPAIGNROOT", self.MY_COMPONENT_UID, "scripts", "emf_helper.sh" )
