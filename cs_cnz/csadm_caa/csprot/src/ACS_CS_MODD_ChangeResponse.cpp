/*
 * ACS_CS_MODD_ChangeResponse.cpp
 *
...
ACS_CS_Trace_TDEF(ACS_CS_MODD_ChangeResponse_TRACE);


ACS_CS_MODD_ChangeResponse::ACS_CS_MODD_ChangeResponse():
	ACS_CS_Primitive(),
	resultCode(ACS_CS_Protocol::MODD_Result_NotSpecified)
{}


ACS_CS_MODD_ChangeResponse::ACS_CS_MODD_ChangeResponse(const ACS_CS_MODD_ChangeResponse &right):
		ACS_CS_Primitive(),
...
ACS_CS_Protocol::CS_Primitive_Identifier ACS_CS_MODD_ChangeResponse::getPrimitiveType () const
{
	return ACS_CS_Protocol::Primitive_MODD_ChangeResponse;
}


int ACS_CS_MODD_ChangeResponse::setBuffer (const char *buffer, int size)
{
	if (buffer == 0)	// Check for valid memory
	{
		ACS_CS_TRACE((ACS_CS_MODD_ChangeResponse_TRACE,
			"(%t) ACS_CS_MODD_ChangeResponse::setBuffer()\n"
			"Error: NULL pointer\n" ));
...
int ACS_CS_MODD_ChangeResponse::getBuffer (char *buffer, int size) const
{
	if (buffer == 0)	// Check for valid memory
	{
		ACS_CS_TRACE((ACS_CS_MODD_ChangeResponse_TRACE,
			"(%t) ACS_CS_MODD_ChangeResponse::getBuffer()\n"
			"Error: NULL pointer\n" ));
...
unsigned short ACS_CS_MODD_ChangeResponse::getLength () const
{
	return this->description.length() + 5;
}


ACS_CS_Primitive * ACS_CS_MODD_ChangeResponse::clone () const
{
	return new ACS_CS_MODD_ChangeResponse(*this);
}
