...
int __CLASS_NAME__::vtrace (const char * file, int line, const char * format, va_list ap) {
	if (_tra_tracer.ACS_TRA_ON()) {
		char trace_msg[ACS_APBM_TRACE_MESSAGE_SIZE_MAX];
		int char_wrote = 0;

		if (file || (line > 0)) {
			char_wrote = ::snprintf(trace_msg, ACS_APBM_ARRAY_SIZE(trace_msg), "{");

			if (file) {
				char * filename = const_cast<char *> (::strrchr(file, '/')); 
				char_wrote += ::snprintf(trace_msg + char_wrote, ACS_APBM_ARRAY_SIZE(trace_msg) - char_wrote, "%s", filename ? (filename + 1) : "NO_FILE");
			}

			(line > 0) && (char_wrote += ::snprintf(trace_msg + char_wrote, ACS_APBM_ARRAY_SIZE(trace_msg) - char_wrote, "::%d", line));
			char_wrote += ::snprintf(trace_msg + char_wrote, ACS_APBM_ARRAY_SIZE(trace_msg) - char_wrote, "} ");
		}

		::vsnprintf(trace_msg + char_wrote, ACS_APBM_ARRAY_SIZE(trace_msg) - char_wrote, format, ap);
		trace_msg[ACS_APBM_ARRAY_SIZE(trace_msg) - 1] = 0;
...
__CLASS_NAME__::__CLASS_NAME__ (acs_apbm_tracer & tracer, const char * function_name)
: _tracer(tracer), _function_name(function_name) {
	_tracer.trace("Entering function '%s'", function_name);
}

__CLASS_NAME__::__CLASS_NAME__ (acs_apbm_tracer & tracer, const char * function_name, const char * file_name, int line)
: _tracer(tracer), _function_name(function_name) {
	_tracer.trace(file_name, line, "Entering function '%s'", _function_name ?: "NO_FUNCTION");
}
