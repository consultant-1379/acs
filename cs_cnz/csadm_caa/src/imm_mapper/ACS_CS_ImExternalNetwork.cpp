...
BOOST_CLASS_EXPORT_IMPLEMENT(ACS_CS_ImExternalNetwork);

ACS_CS_ImExternalNetwork::ACS_CS_ImExternalNetwork()
:adminState(EMPTY_ADMIN_STATE)
{
   type = TRM_EXTERNALNETWORK_T;
}

ACS_CS_ImExternalNetwork::ACS_CS_ImExternalNetwork(const ACS_CS_ImExternalNetwork *other)
{
	copyObj(other);
}
...
bool ACS_CS_ImExternalNetwork::isValid(int & errNo, const ACS_CS_ImModel & model)
{
	errNo = 0;
...
bool ACS_CS_ImExternalNetwork::modify(ACS_APGCC_AttrModification **attrMods)
{
   for (int i = 0; attrMods[i] != NULL; i++)
	{
		...
		if (strcmp(modAttr->attrName, ACS_CS_ImmMapper::ATTR_TRM_EXTERNALNETWORK_ID.c_str()) == 0)
		{
...
ACS_CS_ImBase * ACS_CS_ImExternalNetwork::clone() const
{
   ACS_CS_ImBase *cp = new ACS_CS_ImExternalNetwork(*this);
   return cp;
}


bool ACS_CS_ImExternalNetwork::equals(const ACS_CS_ImBase &obj) const
{
   if (!ACS_CS_ImBase::equals(obj))
	...
	try
	{
		const ACS_CS_ImExternalNetwork & other = dynamic_cast<const ACS_CS_ImExternalNetwork &>(obj);
...
void ACS_CS_ImExternalNetwork::copyObj(const ACS_CS_ImExternalNetwork *other)
{
	rdn = other->rdn;
	type = other->type;
	action = other->action;
...
ACS_CS_ImImmObject * ACS_CS_ImExternalNetwork::toImmObject(bool onlyModifiedAttrs)
{
   ACS_CS_ImImmObject *immObject = new ACS_CS_ImImmObject();

   immObject->setClassName(ACS_CS_ImmMapper::CLASS_TRM_EXTERNALNETWORK.c_str());
   ...
   if(!onlyModifiedAttrs)
   	   attrList.push_back(ACS_CS_ImUtils::createStringType(ACS_CS_ImmMapper::ATTR_TRM_EXTERNALNETWORK_ID.c_str(), networkId));
...
namespace boost {
   namespace serialization {
      template void serialize( boost::archive::text_iarchive & ar, ACS_CS_ImExternalNetwork & base, const unsigned int /*version*/);
      template void serialize( boost::archive::text_oarchive & ar, ACS_CS_ImExternalNetwork & base, const unsigned int /*version*/);
      template void serialize( boost::archive::binary_iarchive & ar, ACS_CS_ImExternalNetwork & base, const unsigned int /*version*/);
      template void serialize( boost::archive::binary_oarchive & ar, ACS_CS_ImExternalNetwork & base, const unsigned int /*version*/);
   }
}
