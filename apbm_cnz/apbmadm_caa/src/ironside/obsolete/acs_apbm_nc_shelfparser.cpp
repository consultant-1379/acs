...
#include "acs_apbm_nc_shelfparser.h"
#include <stdio.h>


acs_apbm_nc_shelfparser::acs_apbm_nc_shelfparser()
{
     resetFlags();   
...
void acs_apbm_nc_shelfparser::warning(const SAXParseException &/*exception*/) {

}

//	Metod to handle errors that occured during parsing

void acs_apbm_nc_shelfparser::error(const SAXParseException &/*exception*/) {
...
void acs_apbm_nc_shelfparser::fatalError(const SAXParseException &/*exception*/) {
...
void acs_apbm_nc_shelfparser::characters(const XMLCh * const chars, const unsigned int length)
{
    if(length<1)
        return;
...
void acs_apbm_nc_shelfparser::startElement(const XMLCh * const name, AttributeList& ) {
...
*/
 ...
 void acs_apbm_nc_shelfparser::getResponse(nresponse &res)
 {
     res = _response;
 }
...
void acs_apbm_nc_shelfparser::resetFlags()
{
     fl_shelfId=false;
     fl_userLabel=false;
     fl_rack=false;
...
void acs_apbm_nc_shelfparser::endElement(const XMLCh* const)
{
