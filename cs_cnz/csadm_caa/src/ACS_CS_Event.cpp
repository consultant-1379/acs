...
ACS_CS_Trace_TDEF(ACS_CS_Event_TRACE);
...
//
ACS_CS_Event::ACS_CS_Event()
{
}


ACS_CS_Event* ACS_CS_Event::instance()
{
    return instance_::instance();
}


ACS_CS_EventHandle ACS_CS_Event::CreateEvent(bool manualReset, bool initialState, const char* name)
{
    ACS_CS_EventHandle eventHandle = -1;
    ...
    if ((eventHandle = eventfd(0, 0)) == -1) {
        ...
        ACS_CS_FTRACE((ACS_CS_Event_TRACE, LOG_LEVEL_ERROR, "(%t) Could not create named event: \"%s\", handle: %d", name, eventHandle));
    ...
    if (name != NULL) {
        ...
        ACS_CS_TRACE((ACS_CS_Event_TRACE,
                "(%t) Created named event: \"%s\", handle: %d", name, eventHandle));
    } else {
        // unnamed event
        instance()->lock_.acquire_write();
        ...
        ACS_CS_TRACE((ACS_CS_Event_TRACE,
                "(%t) Created unnamed event: %d", eventHandle))
    }
...
ACS_CS_EventHandle ACS_CS_Event::OpenNamedEvent(string name)
{
...
void ACS_CS_Event::CloseEvent(ACS_CS_EventHandle eventHandle)
{
    instance()->lock_.acquire_write();
    ...
    if (neit != instance()->namedEvents_.end()) {
        ACS_CS_TRACE((ACS_CS_Event_TRACE,
                "(%t) Removing named event, %s", neit->first.c_str()));
    ...
    if (ueit != instance()->events_.end()) {
        ACS_CS_TRACE((ACS_CS_Event_TRACE,
                "(%t) Removing unnamed event, handle: %d", eventHandle));
...
int ACS_CS_Event::WaitForEvents(int count, int *eventHandles, int timeoutInMilliseconds)
{
    fd_set fds;
    int nfds = 0;
    ...
    if (ret == 0) {
        ...
        return ACS_CS_EVENT_RC_TIMEOUT;
    ...
    if (n != -1)
    {
        ...
        ACS_CS_Event::EventProperties_t *ep = NULL;

        // locate what type of event this is, start in the named events list
        instance()->lock_.acquire_read();
        ACS_CS_Event::NamedEventIteratorType neit = instance()->findNamedEventByHandle(handleActivated);
        ...
        if (ep == NULL) {
        	...
        	instance()->lock_.acquire_read();
        	ACS_CS_Event::EventIteratorType ueit = instance()->findEventByHandle(handleActivated);
        	if (ueit != instance()->events_.end()) {
        		ep = (*ueit);
        	}
            instance()->lock_.release();
...
bool ACS_CS_Event::SetEvent(ACS_CS_EventHandle eventHandle)
{
    ssize_t sz;
    uint64_t n = 1;
    ...
    if (sz != sizeof(uint64_t)) {
        // write fault
        ACS_CS_TRACE((ACS_CS_Event_TRACE, "(%t) Failed to set event for handle: %d", eventHandle));
        return false;
    }
...
bool ACS_CS_Event::ResetEvent(ACS_CS_EventHandle eventHandle)
{
    fd_set fds;
    struct timeval tv;
    ...
    if (ret == -1) {
        ...
        ACS_CS_TRACE((ACS_CS_Event_TRACE,
                "(%t) ResetEvent() returned fault for handle: %d", eventHandle));
...
ACS_CS_Event::NamedEventIteratorType ACS_CS_Event::findNamedEventByHandle(ACS_CS_EventHandle handle)
{
    // check the list with named events
...
// NOTE! Must be wrapped inside acquire/release statement
ACS_CS_Event::EventIteratorType ACS_CS_Event::findEventByHandle(ACS_CS_EventHandle handle)
{
    // check the list of unnamed events
