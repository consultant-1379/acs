...
#include "APZ21240Configuration.H"
...
//    ptbPeer     The PTB peer that tells which physical side's CP
...
//******************************************************************************
APZ21240Configuration::APZ21240Configuration(const char* theName, PTBPeer ptbPeer,
   ...
   m_upbbBoardType(UpbbUnknown),
   m_state(InitialSetup),
   m_CPBB_Eth0State(SendUpbbPromRevisionRequest),
   m_CPBB_Eth1State(SendUpbbPromRevisionRequest),
   m_currentDiscoveryItem(CPBB_Eth0)
{
   DBGLOG("APZ21240Configuration(%s, ...)\n",
          name());
   ...
   for(int item = CPBB_Eth0; item < NumberOfDiscoveryItems; ++item)
   {
...
//    The Destructor.
//******************************************************************************
APZ21240Configuration::~APZ21240Configuration()
{
}

//******************************************************************************
// Description:
//    Checks the APZ 212 40 DHCP basic configuration.
//    The APZ 212 40 scope configuration is static and thereby configurated once
//    is enough. The method checks that unique APZ 212 40 information exist. 
//    If the check is false, it's interpreted as the DHCP is not configured for 
//    APZ 212 40 and this will be done. The scope, iprange and excludes will be
...
//    APZ 212 40 setup.
//******************************************************************************
APZConfiguration::Operation APZ21240Configuration::checkBasicDHCPSetupImplementation()
{
...
/*
   if(m_dhcpConfiguration.existsScope(Eth0_IPAddress_CPBB_A & netmask, 
...
/*
      ...
      if(m_dhcpConfiguration.existsIPRange(Eth0_IPAddress_CPBB_A & netmask, 
         Eth0_IPAddress_CPBB_A, // Lower IP range
         isFound) != DHCPConfiguration::OK)
      {
...
*/
      if(isFound == true)
      {
         ...
         // specific APZ 212 40 configuration
         // Build up a temporarily client
         // Client information for interface Eth0 for the CPBB.
         ClientInformation aCPBBClient;
         // Assign hostnames.
         aCPBBClient.hostname("eib_a");
         // Assign descriptions.
         aCPBBClient.description("");
         // Assign IP address information.
         aCPBBClient.ipAddress(Eth0_IPAddress_CPBB_A);

         // Assign boot image file names.
         static_cast<void>(aCPBBClient.bootFilename("image\\boot_image_cpa"));
         // Assign boot IP address.
         aCPBBClient.bootIPAddress(m_BootIPAddress_Lan1);
/*
         if(m_dhcpConfiguration.existsClientsBootfile(aCPBBClient, isFound) != DHCPConfiguration::OK)
         {
            DBGLOG("(%s) Failed to check for client's boot file name: %s/%s\n", name(),
               aCPBBClient.hostname(),aCPBBClient.bootFilename());
            return Failed;
         }
 */
      }
   ...
   {
      // Primary scope
      // Add primary scope. Implicit delete of current primary scope is first done.
      ...
      if(m_dhcpConfiguration.addIpRange(Eth0_IPAddress_CPBB_A & netmask, 
         Eth0_IPAddress_CPBB_A & 0xFF, // Lower IP range 
         HighIPRange, 
      ...
      // Add exclude range for primary scope
      if(m_dhcpConfiguration.addExcludeRange(Eth0_IPAddress_CPBB_A & netmask, 
         ExcludeRangeLow, ExcludeRangeHigh) != DHCPConfiguration::OK)
      {
         DBGLOG("(%s) Failed to set the primary scope's (ipna) exclude range\n", name());
         return Failed;
      }
      ...
      if(m_dhcpConfiguration.addIpRange(Eth1_IPAddress_CPBB_A & netmask,
         Eth1_IPAddress_CPBB_A & 0xFF, // Lower IP range
      ...
      // Add exclude range for secondary scope
      if(m_dhcpConfiguration.addExcludeRange(Eth1_IPAddress_CPBB_A & netmask, 
         ExcludeRangeLow, ExcludeRangeHigh) != DHCPConfiguration::OK)
      {
         DBGLOG("(%s) Failed to set the secondary scope's (ipnb) exclude range\n", name());
         return Failed;
      }
   }
   // else
...
//******************************************************************************
APZConfiguration::Operation APZ21240Configuration::cleanUpDHCPSetupImplementation()
{
   u_int32 netmask = m_dhcpConfiguration.netmask();
...
//    client.
//******************************************************************************
APZConfiguration::Operation APZ21240Configuration::initializeImplementation()
{
   ...
   if(!m_isClientLeaseConfigured)
   {
      ...
      if(m_dhcpConfiguration.setOptionValue(Eth0_IPAddress_CPBB_A & netmask, 51, 4294967295) != DHCPConfiguration::OK ||
         m_dhcpConfiguration.setOptionValue(Eth0_IPAddress_CPBB_A & netmask, 51, 4294967295, "Default BOOTP Class") != DHCPConfiguration::OK)
      {
      ...
      if(m_dhcpConfiguration.setOptionValue(Eth1_IPAddress_CPBB_A & netmask, 51, 4294967295) != DHCPConfiguration::OK ||
         m_dhcpConfiguration.setOptionValue(Eth1_IPAddress_CPBB_A & netmask, 51, 4294967295, "Default BOOTP Class") != DHCPConfiguration::OK)
      {
   ...
   if(!m_isDhcpRegistryConfigured && (m_PTBPeer == PTBB))
   {
      ...
      u_int32 keyValue = 0; // The value of the IgnoreBroadcastFlag for APZ 212 40
...
//    Finished.
//******************************************************************************
APZConfiguration::Operation APZ21240Configuration::setupPeerConnectionImplementation()
{
...
//******************************************************************************
APZConfiguration::Operation APZ21240Configuration::setupDHCPImplementation()
{
...
//    Finished.
//******************************************************************************
APZ21240Configuration::Operation APZ21240Configuration::finalizeImplementation()
{
   keepAlive();
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::connectToPTB()
{
   DBGLOG("(%s) connectToPTB\n", name());

   APZ21240Configuration::Status reply = NotOK;
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::setupDHCPInformtion()
{
   ...
   if(m_currentDiscoveryItem == AllItemsProcessed)
   {
      ...
      // Activate the scopes. Let only one APZ21240Configuration thread do the job.
      if(m_PTBPeer == PTBB)
      {
   ...
   {
   case CPBB_Eth0:
      discoveryState = &m_CPBB_Eth0State;
      orderCode = CPTREADMAUREG::CPBB_ETH0;
      // Swap the OrderCode if connected to a UPBB version 2
      if(m_upbbBoardType == Upbb2)
         orderCode = CPTREADMAUREG::CPBB_ETH1;
      clientInformation = &m_CPBB_Eth0;
      break;

   case CPBB_Eth1:
      discoveryState = &m_CPBB_Eth1State;
      orderCode = CPTREADMAUREG::CPBB_ETH1;
      // Swap the OrderCode if connected to a UPBB version 2
      if(m_upbbBoardType == Upbb2)
         orderCode = CPTREADMAUREG::CPBB_ETH0;
      clientInformation = &m_CPBB_Eth1;
      break;
   ...
   {
   case SendUpbbPromRevisionRequest:
      if(sendCPTREADMAUREG(CPTREADMAUREG::UPBB_PROM_REVISION) == OK)
      {
         *discoveryState = ReadUpbbPromRevisionReply;
      }
      else
      {
         return NotOK;
   ...
   case ReadUpbbPromRevisionReply:
      // Read the reply to the requested board type (this will change the
      // state to  SendMACAddressRequest if successful, and set the 
      // m_upbbBoardType to a state !UpbbUnknown).
      if(readCPTREADMAUREG(CPTREADMAUREG::UPBB_PROM_REVISION, discoveryState) != OK)
      {
         return NotOK;
      }
      break;
...
//******************************************************************************
void APZ21240Configuration::keepAlive()
{
   DBGLOG("(%s) keepAlive\n", name());
...
//******************************************************************************
ClientInformation* APZ21240Configuration::getClientInformationImplementation(const MACAddress& macAddress)
{
   DBGLOG("(%s) getClientInformationImplementation\n", name());

   if(m_CPBB_Eth0.isMACAddressInformationKnown() &&
      macAddress == MACAddress(m_CPBB_Eth0.macAddress()))
   {
      return &m_CPBB_Eth0;
   }

   if(m_CPBB_Eth1.isMACAddressInformationKnown() &&
      macAddress == MACAddress(m_CPBB_Eth1.macAddress()))
   {
      return &m_CPBB_Eth1;
   }
...
//    has reached zero.
//******************************************************************************
void APZ21240Configuration::decreaseRevertTimerImplementation(u_int16 elapsedSeconds)
{
   decreaseRevertTimer(elapsedSeconds, m_CPBB_Eth0);
   decreaseRevertTimer(elapsedSeconds, m_CPBB_Eth1);
 }
...
//******************************************************************************
void APZ21240Configuration::automaticRevertImplementation()
{
   automaticRevert(m_CPBB_Eth0);
   automaticRevert(m_CPBB_Eth1);
}
...
//******************************************************************************
APZConfiguration::Operation APZ21240Configuration::changeBootFilenameImplementation(const ClientInformation* client,
...
//******************************************************************************
void APZ21240Configuration::ceaseAlarmsImplementation()
{
   ceasePTBConnectionAlarm();
   ceaseInvalidCPTREADMAUREGRAlarm(CPBB_Eth0);
   ceaseInvalidCPTREADMAUREGRAlarm(CPBB_Eth1);
   ceaseCPTREADMAUREGRParseErrorAlarm(CPBB_Eth0);
   ceaseCPTREADMAUREGRParseErrorAlarm(CPBB_Eth1);
}
...
//******************************************************************************
void APZ21240Configuration::assignStaticClientInformation()
{
   ...
   if(m_PTBPeer == PTBA)
   {
      ...
      // Assign hostnames.
      m_CPBB_Eth0.hostname("eib_a");
      m_CPBB_Eth1.hostname("eic_a");

      // Assign descriptions.
      m_CPBB_Eth0.description("CP-AP Lan1");
      m_CPBB_Eth1.description("CP-AP Lan2");

      // Assign IP address information.
      m_CPBB_Eth0.ipAddress(Eth0_IPAddress_CPBB_A);
      m_CPBB_Eth1.ipAddress(Eth1_IPAddress_CPBB_A);
   
      // Assign boot image file names.
      static_cast<void>(m_CPBB_Eth0.bootFilename("image\\boot_image_cpa"));
      static_cast<void>(m_CPBB_Eth1.bootFilename("image\\boot_image_cpa"));

      // Assign boot IP address.
      m_CPBB_Eth0.bootIPAddress(m_BootIPAddress_Lan1);
      m_CPBB_Eth1.bootIPAddress(m_BootIPAddress_Lan2);
   }
   else if(m_PTBPeer == PTBB)
   {
      ...
      // Assign hostnames.
      m_CPBB_Eth0.hostname("eib_b");
      m_CPBB_Eth1.hostname("eic_b");

      // Assign descriptions.
      m_CPBB_Eth0.description("CP-AP Lan4");
      m_CPBB_Eth1.description("CP-AP Lan3");

      // Assign IP address information.
      m_CPBB_Eth0.ipAddress(Eth0_IPAddress_CPBB_B);
      m_CPBB_Eth1.ipAddress(Eth1_IPAddress_CPBB_B);
 
      // Assign boot image file names.
      static_cast<void>(m_CPBB_Eth0.bootFilename("image\\boot_image_cpb"));
      static_cast<void>(m_CPBB_Eth1.bootFilename("image\\boot_image_cpb"));

      // Assign boot IP address.
      m_CPBB_Eth0.bootIPAddress(m_BootIPAddress_Lan2);
      m_CPBB_Eth1.bootIPAddress(m_BootIPAddress_Lan1);
   }
   else
   {
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::addStaticGESBEntries()
{
   // Let only one APZ21240Configuration thread set up the GESBs'
   // DHCP entries.
   if(m_PTBPeer == PTBB)
   {
      return OK;
...
//******************************************************************************
void APZ21240Configuration::automaticRevert(ClientInformation& clientInformation)
{
...
//    has reached zero.
//******************************************************************************
void APZ21240Configuration::decreaseRevertTimer(u_int16 elapsedSeconds,
...
//******************************************************************************
void APZ21240Configuration::clearSetupDHCPInformation()
{
   ...
   // Begin with acquireing information from CPBB_Eth0.
   m_currentDiscoveryItem = CPBB_Eth0;
}
...
// Returns:
//    Either of CPBB_Eth0 or CPBB_Eth1 when further
//    discovery is required. AllItemsProcessed when all information has been
//    acquired and set up in the DHCP server.
//******************************************************************************
APZ21240Configuration::DiscoveryItem APZ21240Configuration::nextDiscoveryItem()
{
   static bool isInvalidStateReported = false;
   ...
   {
   case CPBB_Eth0:
      if(isInEndState(m_CPBB_Eth0State))
      {
         if(!isInEndState(m_CPBB_Eth1State))
         {
            nextItem = CPBB_Eth1;
         }
         else
...
//******************************************************************************
void APZ21240Configuration::raisePTBConnectionAlarm()
{
...
//******************************************************************************
void APZ21240Configuration::ceasePTBConnectionAlarm()
{
   if(m_isConnectionAlarmSet)
   {
...
//******************************************************************************
void APZ21240Configuration::raiseInvalidCPTREADMAUREGRAlarm(CPTREADMAUREG::OrderCode orderCode,
                                                            ...
                                                            DiscoveryItem discoveryItem)
{
   ...
   *discoveryState = Stuck;
   ...
   if(discoveryItem == CPBB_Eth0 ||
      discoveryItem == CPBB_Eth1)
   {
...
//******************************************************************************
void APZ21240Configuration::ceaseInvalidCPTREADMAUREGRAlarm(DiscoveryItem discoveryItem)
{
   static bool isInvalidDiscoveryItemReported = false;
...
//    discoveryItem   The DiscoveryItem for which the alarm should be raised.
//******************************************************************************
void APZ21240Configuration::raiseParseCPTREADMAUREGRErrorAlarm(CPTREADMAUREG::OrderCode orderCode,
                                                               const CPTREADMAUREGR* cptreadmauregr,
                                                               DiscoveryState* discoveryState,
                                                               DiscoveryItem discoveryItem)
{
...
//    discoveryItem  The DiscoveryItem for which the alarm should be ceased.
//******************************************************************************
void APZ21240Configuration::ceaseCPTREADMAUREGRParseErrorAlarm(DiscoveryItem discoveryItem)
{
   static bool isInvalidDiscoveryItemReported = false;
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::send(const char* data,
                                                          u_int32 dataSize)
{
   u_int32 sentSize = 0;
   u_int32 size;
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::read(char* data,
                                                          u_int32 maxSize,
                                                          u_int32& readSize)
{
...
//    disconnected in the middle of the sending process.
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::sendCPTREADMAUREG(CPTREADMAUREG::OrderCode orderCode)
{
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::readCPTREADMAUREG(CPTREADMAUREG::OrderCode orderCode,
                                                                       ClientInformation* clientInformation,
                                                                       DiscoveryState* discoveryState)
{
      ...
      cptreadmauregr->numberOfData() == 14) // A valid reply always has 14 byte worth of data.
   {
      ...
      if(converted == 6)
      {
         ...
         DBGLOG("(%s) Read MAC address %02X:%02X:%02X:%02X:%02X:%02X for %s is assigned to client IP %d.%d.%d.%d\n",
                name(),
                a, b, c, d, e, f,
                (orderCode == CPTREADMAUREG::CPBB_ETH0 ? "CPBB_Eth0" : orderCode == CPTREADMAUREG::CPBB_ETH0 ? "CPBB_Eth1" : "Unknown"),
...
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::configureDHCP(ClientInformation* clientInformation)
{
...
// Description:
//    Attempts to read a CPTREADMAUREG signal that should match the supplied
//    orderCode with the supplied order code to the
//    destination PTB peer.
// Parameters:
//    orderCode          Defines a request to read the UPBB Prom revision (22).
//                       With the revision, the type of UPBB board is found.
//    discoveryState     Will change to SendMACAddressRequest if a match is found.
// Returns:
//    NotOK if we got disconnected in the middle of the receiving process.
//******************************************************************************
APZ21240Configuration::Status APZ21240Configuration::readCPTREADMAUREG(CPTREADMAUREG::OrderCode orderCode,
                                                                       DiscoveryState* discoveryState)
{
   ...
   // Proceed to validation of the reply.
   // Check the received UPBB prom revision.
   if(cptreadmauregr->signalNumber() == CPTREADMAUREGR::SignalNumber &&
      cptreadmauregr->faultCode() == CPTREADMAUREGR::NoFault &&
      cptreadmauregr->orderCode() == orderCode)
      //&& cptreadmauregr->numberOfData() == 34) // A valid reply always has 34 byte worth of data.
   {
      // Cease a previously alarmed state (if required).
      ceaseInvalidCPTREADMAUREGRAlarm(m_currentDiscoveryItem);

      // Note, the MACAddress method is used for retrieving the data.
      // In this case the upbb prom revison. The string looking for shows
      // the UPBB_V24 firmware. 
      char searchstr[35]; // READMAUREGR_DATA_S is max 34 plus 1 for NULL
      memcpy(searchstr, cptreadmauregr->MACAddress(), cptreadmauregr->numberOfData());
      searchstr[cptreadmauregr->numberOfData()] = NULL;

      if (strstr(searchstr, "CXC1060117") != NULL) // UPBB1
      {
         m_upbbBoardType = Upbb1;
         // Advance to the next state of operation.
         *discoveryState = SendMACAddressRequest;
      }
      else if(strstr(searchstr, "CXC1060153") != NULL) // UPBB2
      {
         m_upbbBoardType = Upbb2;
         // Advance to the next state of operation.
         *discoveryState = SendMACAddressRequest;
      }
      else
      ...
      DBGLOG("(%s) Read UPBB prom revision: %s, which is interpreted as UPPB board type: %d\n",
                name(), searchstr, m_upbbBoardType);
