...
def createSMFCampaignPlugin():
   return LMSMFCampaignPlugin()

class LMSMFCampaignPlugin(SMFCampaignPlugin):
   # initialization method
   # self: variable represents the instance of the object itself
   def __init__(self):
      super(LMSMFCampaignPlugin, self).__init__()
      self.MY_COMPONENT_UID = "acs.lm"
      self._info = None
      self._utils = None
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
   def cliAtCampInit(self):
      ...
      lm = self._info.getComponent("acs.lm")
      for bundle in lm.swBundles:
         bundleName = bundle[1] 
      result = []
      ...
      if (self._actionType == SMFConstants.CT_INSTALL or self._actionType == SMFConstants.CT_UPGRADE or self._actionType == SMFConstants.CT_MIGRATE):
         cli = os.path.join("$OSAFCAMPAIGNROOT", self.MY_COMPONENT_UID, "scripts", "lm_helper.sh" )
