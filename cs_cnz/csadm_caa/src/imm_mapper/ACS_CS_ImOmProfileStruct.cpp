...
BOOST_CLASS_EXPORT_IMPLEMENT(ACS_CS_ImOmProfileStruct);

ACS_CS_ImOmProfileStruct::ACS_CS_ImOmProfileStruct():
axeCpClusterStructId(""),actionId(UNDEFINED_TYPE),state(UNDEFINED_STATE),
result(NOT_AVAILABLE),reason(""), timeOfLastAction("")
{
   type = OMPROFILEMANAGERSTRUCT_T;
}

ACS_CS_ImOmProfileStruct::ACS_CS_ImOmProfileStruct(const ACS_CS_ImOmProfileStruct &other)
{
   *this = other;
}
...
bool ACS_CS_ImOmProfileStruct::isValid(int & errNo, const ACS_CS_ImModel &/*model*/)
{
   errNo = 0;
   return true;
}

bool ACS_CS_ImOmProfileStruct::modify(ACS_APGCC_AttrModification **attrMods)
{
...
ACS_CS_ImBase * ACS_CS_ImOmProfileStruct::clone() const
{
   ACS_CS_ImBase *bladeClusterInfo = new ACS_CS_ImOmProfileStruct(*this);
   return bladeClusterInfo;
}


ACS_CS_ImOmProfileStruct & ACS_CS_ImOmProfileStruct::operator=(const ACS_CS_ImOmProfileStruct &rhs)
{
   copyObj(rhs);
   return *this;
}
...
ACS_CS_ImOmProfileStruct & ACS_CS_ImOmProfileStruct::copyObj(const ACS_CS_ImOmProfileStruct &object)
{
...
bool ACS_CS_ImOmProfileStruct::equals(const ACS_CS_ImBase &obj) const
{
   if (!ACS_CS_ImBase::equals(obj))
      return false;

   try {
      const ACS_CS_ImOmProfileStruct & other = dynamic_cast<const ACS_CS_ImOmProfileStruct &>(obj);
...
ACS_CS_ImImmObject * ACS_CS_ImOmProfileStruct::toImmObject(bool onlyModifiedAttrs)
{

   ACS_CS_ImImmObject *immObject = new ACS_CS_ImImmObject();

   immObject->setClassName(ACS_CS_ImmMapper::CLASS_OM_PROFILE_STRUCT.c_str());
...
namespace boost {
   namespace serialization {
      template void serialize( boost::archive::text_iarchive & ar, ACS_CS_ImOmProfileStruct & base, const unsigned int /*version*/);
      template void serialize( boost::archive::text_oarchive & ar, ACS_CS_ImOmProfileStruct & base, const unsigned int /*version*/);
      template void serialize( boost::archive::binary_iarchive & ar, ACS_CS_ImOmProfileStruct & base, const unsigned int /*version*/);
      template void serialize( boost::archive::binary_oarchive & ar, ACS_CS_ImOmProfileStruct & base, const unsigned int /*version*/);
   }
}
