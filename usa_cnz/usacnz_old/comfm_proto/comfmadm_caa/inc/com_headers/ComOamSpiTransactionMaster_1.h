...
typedef struct ComOamSpiTransactionMaster_1 {
     ...
     * The max value for timeout is 3600.
     ...
     *
     * @b Details: <br>
     * Prepare will be called on the participants.
     * If a participant responds with an error code then prepare will
     * not be called on the remaining participants and abort will be
     * called on all participants.
     * If all participants succeed then commit will always be called
     * on all participants.
     * If one or more participants commit fails then the potential
     * resulting inconsistency must be resolved by a human.
     * At last, finish is called on all participants, giving them
