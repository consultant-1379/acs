#ifndef HEADER_GUARD_CLASS__ACS_DSD_Sender
#define HEADER_GUARD_CLASS__ACS_DSD_Sender ACS_DSD_Sender

/** @file ACS_DSD_Sender.h
 *	@brief
...
#define __CLASS_NAME__ HEADER_GUARD_CLASS__ACS_DSD_Sender

#ifdef ACS_DSD_API_TRACING_ACTIVE
extern ACS_DSD_API_CLASS_TRACER_DECL(__CLASS_NAME__);
#endif

/** @class ACS_DSD_Sender ACS_DSD_Sender.h
 *	@brief ACS_DSD_Sender class
 *	@author xnicmut (Nicola Muto)
 *	@date 2010-11-09
 *	@version 1.0.1
 *
 *	ACS_DSD_Sender Class detailed description
 */
class __CLASS_NAME__ {
	...
	//==============//
protected:
	/** @brief ACS_DSD_Sender Default constructor
	 *
	 *	ACS_DSD_Sender Constructor detailed description
	 *
	...
	/** @brief ACS_DSD_Sender Copy constructor
	 *
	 *	ACS_DSD_Sender Copy Constructor detailed description
	 *
	 *	@remarks Remarks
	 */
	...
	//============//
public:
	/** @brief ACS_DSD_Sender Destructor
	 *
	 *	ACS_DSD_Sender Destructor detailed description
	 *
	 *	@remarks Remarks
	 */
...
public:
	 ...
	 *	@remarks Remarks
	 */
//	virtual ssize_t send (const void * buf, size_t len, const ACE_Time_Value * timeout = 0, size_t * bytes_transferred = 0) const = 0;
	inline ssize_t send (const void * buf, size_t len, const ACE_Time_Value * timeout = 0, size_t * bytes_transferred = 0) const {
		return sendf(buf, len, 0, timeout, bytes_transferred);
	}

	virtual ssize_t sendf (const void * buf, size_t len, int flags, const ACE_Time_Value * timeout = 0, size_t * bytes_transferred = 0) const = 0;
...
#endif // HEADER_GUARD_CLASS__ACS_DSD_Sender
